...
description: How JMDT uses Zero-Knowledge Proofs (zk-STARKs) via the RISC Zero zkVM and Rust-based circuits to batch transactions privately and anchor block commitments to Ethereum via the ZK rollup contract.
keywords: [Zero-Knowledge Proofs, ZKP, zk-STARKs, RISC Zero zkVM, JMDT ZK, private transactions, Ethereum ZK, L2 ZK proofs, Rust ZK circuits]
---
...
## STARK Proofs via the RISC Zero zkVM

JMDT's proof system is built on **zk-STARKs** (Scalable Transparent Arguments of Knowledge), generated by the **RISC Zero zkVM**:
...
- Used for: transaction-level validation, DID authentication, L2 state checks, and the L2 → L1 anchoring pipeline (full state-transition proving being completed); with the planned L3, DAG state commitments
